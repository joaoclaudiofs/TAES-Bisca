...
class VerifyDataStorageUS25 {
	...
	@When("Eu clico no botão {string} 25")
	def eu_clico_no_botao(String string) {
		Mobile.scrollToText("My Stats")
		TestObject btn = findTestObject('Object Repository/android.widget.TextView - ' + string)
		Mobile.waitForElementPresent(btn, 10)
		Mobile.tap(btn, 0)
	}


	@Then("Eu vejo o texto {string} e as minhas estatísticas 25")
	def eu_vejo_o_texto(String string) {
		...
		Mobile.verifyElementExist(findTestObject('Object Repository/android.widget.TextView - Wins'), 0)
		Mobile.verifyElementExist(findTestObject('Object Repository/android.widget.TextView - Losses'), 0)
		["Wins", "Losses"].each { label ->
			TestObject numberObj = getStatNumberObj(label)
			Mobile.verifyElementExist(numberObj, 5)
			String numberText = Mobile.getText(numberObj, 5)
			println(label + " = " + numberText)
		}
	}
	...
	def getStatNumberObj(String label) {
		TestObject obj = new TestObject("NumberAbove_" + label)
		obj.addProperty("xpath", ConditionType.EQUALS,
				"//android.widget.TextView[@text='" + label + "']/preceding-sibling::android.widget.TextView[1]")
		return obj
	}
}